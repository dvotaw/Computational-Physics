...
// 
//  Functions to generate normalized harmonic oscillator wavefunctions.
//
...
double ho_radial(int n, int l, double b, double r)
{
  ...
  return norm(n, l, b) * gsl_pow_int(q, (l + 1)) * exp(-qsq / 2.) * gsl_sf_laguerre_n((n - 1), a, qsq);
}
...
double norm(int n, int l, double b)
{
  ...
  return sqrt(2. * gsl_sf_fact((unsigned) (n - 1)) / (b * gsl_sf_gamma (arg)));
}
