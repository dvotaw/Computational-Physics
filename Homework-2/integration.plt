...
# Fit functions.
f1(x) = a1*x + b1
fit [0.1:3] f1(x) "integration_results.txt" using ($1):($2) via a1,b1 
fit_title1 = sprintf("%-+4.1f*x %-+4.1f",a1,b1)

f2(x) = a2*x + b2
fit [0.1:2] f2(x) "integration_results.txt" using ($1):($3) via a2,b2 
fit_title2 = sprintf("%-+4.1f*x %-+4.1f",a2,b2)

#f3(x) = a3*x + b3
#fit [0.1:8] f3(x) "integration_results.txt" using ($1):($4) via a3,b3 
...
plot "integration_results.txt" using ($1):($2) title 'Simpson', \
     ...
     "integration_results.txt" using ($1):($3) title 'Milne', \
     a2*x + b2 title fit_title2, \
     "integration_results.txt" using ($1):($4) title 'GSL QAGS'
#     "integration_results.txt" using ($1):($4) title 'GSL QAGS', \
